...
datapath = '/freespace/local/yh482' # please specific data path
batchSize = 1000
learnRate = 0.001
...
ep = 50 # number of epoches for training
subkeyByte = 2 # index of the attacked subkey byte
...
#    trace = trace['trace']
    print('reading trace...')
    traceframe = pd.read_csv(os.path.join(datapath, 'AES128.txt'), sep=',', header=None)
    trace = traceframe.values
    trace = trace[:, 1:].T
    print('reading text...')
    textframe = pd.read_csv(os.path.join(datapath, 'AES128_ciphertext_plaintext.txt'), sep=' ', header=None)
    text_hex = textframe.values
    text_hex = text_hex[:, :-1]
    text = np.zeros([9000, 32])
    for i in range(9000):
        for j in range(32):
            text[i, j] = int(text_hex[i, j], 16)
    print('reading test trace...')
    traceframe = pd.read_csv(os.path.join(datapath, 'AES128_Test.txt'), sep=',', header=None)
    ...
    y_train = torch.LongTensor(pwr_model(text[:numSample-numValidate, subkeyByte].astype(int), key[subkeyByte]))
    
    X_validate = torch.Tensor(trace[numSample-numValidate:numSample, :lenSignal])
    X_validate = torch.unsqueeze(X_validate, dim=1)
    y_validate = torch.LongTensor(pwr_model(text[numSample-numValidate:numSample, subkeyByte].astype(int), key[subkeyByte]))
...
#            correct = pred.eq(target.data.view_as(pred)).long().cpu().sum()
            print('Train Epoch: {} [{}/{} ({:.0f}%)]\tLoss: {:.6f} validation accuracy: {:.2f}%'.format(
...
# load data
print('reading text...')
textframe = pd.read_csv(os.path.join(datapath, 'AES128_ciphertext_plaintext_Test.txt'), sep=' ', header=None)
...
plt.figure()
plt.plot(mle)
plt.xlabel('Subkey byte guess')
plt.ylabel('Log likelihood score')
plt.show()
recovered_key = np.argmax(mle)
print('recoverd #{} subkey byte is {:s}\n'.format(subkeyByte, hex(recovered_key)))
